 ...
 *
 */
public class DeformToAlignedCSV2 {
	...
	public static void main(final String... args) throws Exception {
		...
		try (final BufferedWriter writer = Files.newBufferedWriter(outPath, Charset.forName("UTF-8"), StandardOpenOption.CREATE, StandardOpenOption.WRITE, StandardOpenOption.TRUNCATE_EXISTING)) {
			...
			writer.write("skeleton_id, treenode_id, parent_treenode_id, x, y, z, r\n");

			for (final CSVRecord csvRecord : parser) {

				try {
					final double[] coordinate = new double[]{
							Double.parseDouble(csvRecord.get(3)) / resolution[0] - offset[0],
							Double.parseDouble(csvRecord.get(4)) / resolution[1] - offset[1],
							Double.parseDouble(csvRecord.get(5)) / resolution[2] - offset[2]};
